...
class ScheduleCog(commands.Cog):
    ...
    async def schedule(self, ctx: commands.Context, time: str = None) -> None:
        ...
        await send(f"{ctx.author.mention} Daily summary scheduled at **{time}**.")
    ...
    async def unschedule(self, ctx: commands.Context) -> None:
        ...
        if count == 0:
            await send(f"{ctx.author.mention} You have no active schedule to cancel.")
        else:
            await send(f"{ctx.author.mention} Daily summary cancelled.")
